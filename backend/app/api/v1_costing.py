...
def read_costing_method(db: Session = Depends(get_db)):
    method = InventoryCostService.get_costing_method(db)
    config = db.query(type(InventoryCostService).__dict__.get("CostingConfig")).first()
...
def update_costing_method(data: CostingConfigUpdate, db: Session = Depends(get_db)):
