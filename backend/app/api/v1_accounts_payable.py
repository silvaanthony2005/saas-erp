...
def read_accounts_payable_by_id(ap_id: int, db: Session = Depends(get_db)):
...
def make_payment(data: PaymentScheduleCreate, db: Session = Depends(get_db)):
...
def read_payment_schedules(ap_id: int, db: Session = Depends(get_db)):
...
def read_cxp_summary(db: Session = Depends(get_db)):
    return PurchaseService.get_cxpsummary(db)

@router.get("/supplier/{supplier_id}/balance", response_model=SupplierBalance)
def read_supplier_balance(supplier_id: int, db: Session = Depends(get_db)):
