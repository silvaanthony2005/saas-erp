...
def pay_receivable(receivable_id: int, payment: PaymentCreate, db: Session = Depends(get_db)):
...
def get_payment_history(receivable_id: int, db: Session = Depends(get_db)):
...
def get_schedule(receivable_id: int, db: Session = Depends(get_db)):
    return CxCService.get_schedule(db, receivable_id)

@router.post("/{receivable_id}/schedule")
def setup_schedule(receivable_id: int, data: ReceivableScheduleSetup, db: Session = Depends(get_db)):
